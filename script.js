...
const finalScoreTextElem = document.querySelector("[data-final-score]");
let lastTime, speedScale, score;

const setPixelToWorldScale = () => {
  let worldToPixelScale;

  if (window.innerWidth / window.innerHeight < WORLD_WIDTH / WORLD_HEIGHT) worldToPixelScale = window.innerWidth / WORLD_WIDTH;
...
const handleStart = (event) => {
  ...
  finalScoreTextElem.classList.add("hide");
  window.requestAnimationFrame(update);
};
...
const handleLose = () => {
  ...
  setTimeout(() => {
    ...
    finalScoreTextElem.classList.remove("hide");
  }, 100);
};
